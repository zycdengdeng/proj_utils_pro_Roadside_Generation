...
`--points-only`：纯净点云图——只保留雷达点，不画坐标轴/标题/图例/标注框（自车框也不画）。

`--target-gap <ms>`：期望的车端−路侧**有符号**时间差（正=车端晚于路侧）。选帧时挑 `gap`
最接近此值的帧，而非一味 `|gap|→0`。车速快、两端有系统延迟导致错位时，用它补偿
（如 `--target-gap 5` 让车端比路侧晚约 5ms；可正可负，自己试 ±）。
